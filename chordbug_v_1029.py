...
from chord_classes2 import Filter, scan, printMidiChannels
from chord_classes2 import UserInteraction as ua
...
def main():
    ...
    ses._addport(ports)
    ses._addFilter(filterMidi)
    ses._addChordScheme("")
    ses._addControlScheme("")
    ses._addUserData(SessionData) #smaller things that are used during session, index, channel, ...
    ...
    rv=ua.readportnumbers()  #reads port indexes from user
    ports.openOutPort(rv.out_index) #usually there is always an outport on a computer
    
    ua.askuseropenports(rv, midi, filterMidi, ports)

...
def callbackChord_Control_Buttons(msg):
    ...
    ## filter out the wrong channels 
    
    if midifilterchannel(msg, midi):
...
def callbackBass(msg):  
    ...
    def midifilterchannel(msg, midiobject):
        # the midiobject is the midi object's instance, currently global .... not good?
        return msg.channel == midiobject._midichannel_bass
    ...
    if msg.type == MidimsgType.note_on:
        ...
        if midifilterchannel(msg, midi): 
            print('midifilterchannel ignored noteon - callbackBass ')
            return #change midi to glob_midi_inst (name)?
...
def main():
    ...
    default_midi_channel_in_bass=2
    default_midi_channel_in_control_chords=0

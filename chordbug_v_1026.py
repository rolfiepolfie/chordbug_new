...
class Globals():
    ...
    note_on_off_flag=False #bass note is down or up 
    global_bassdown=False
...
def callbackChord_Control_Buttons(msg):
    ...
    if mess.isnoteOn(msg):
        ...
        #scan loads a chord-control and place it in the global class
        scan(MidimsgType.note_on, msg.note ,Globals, mess, _callback_scan)
        
        activateChord() # also do this for CC messages 
...
def callbackBass(msg):  
    ...
    '''
    #if bassMiditrough:
    midi.sendMessageOut(msg) #mimics the same as midi-through ... here unfiltered 
...
def main():
    ### ch 9 are sent from T4 as drums
    ### 8,9,10,11 as accomp ... maybe more  
    ...
    while True: # read in a midi port device 
        try:
            ...
            in_index_control = int(input("For control input, enter a number(default 3): ") or "3") #0

...
def main():
    ...
    print("all possible controls that can be handled by assigning to a midi message ") 
    Controls.printall(Controls)
    print("\n -- chords class structure (chord that can be invoked by the system): ") 
    ...
    programnr=midi.sendProgamChange(GmSounds.FX3crystal)
    ...
    midi.testOut()   
    # p=input("press a key to poll input devices: (q for quit)") 
    ...
    #midi.startLoop() #how to break loop and continue to close?
     
    midi.startLoop_keyboardlistener() #
    #Misc.clearSpyderTerminal()
